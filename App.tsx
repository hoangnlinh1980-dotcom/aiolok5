...
const ZALO_GROUP_URL = "https://zalo.me/g/swcazixnkwb1txhvszii";
const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbypK8dtAz3nvEtkA9wVTANKobFWq1v-e7jmVuafyObiOSRZAf-39V82WLx30NcuxWMq3w/exec"; // Dán URL Web App của bạn vào đây sau khi Deploy GAS
...
const COACH = {
  ...
  image: "https://i.postimg.cc/rw0h57r8/HNP05223.jpg",
  bio: "Xin chào, tôi là Linh – người bạn đồng hành mới của bạn. Tôi sẽ sát cánh, hỗ trợ và giải đáp mọi thắc mắc của bạn cho đến khi bạn thực sự làm chủ được công nghệ và hoàn thành mục tiêu trong suốt chương trình này."
};
...
const RegistrationModal = ({ isOpen, onClose, utm }: { isOpen: boolean; onClose: () => void; utm: string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Vẫn cho là thành công để không chặn khách hàng vào nhóm Zalo nếu chỉ lỗi lưu sheet
      setSuccess(true);
    } finally {
      setLoading(false); 
    }
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Coach Section */}
      <section id="coach" className="section-padding px-6 bg-apple-gray/30 border-t border-apple-gray">
        <div className="max-w-[1200px] mx-auto">
          <div className="apple-card p-8 md:p-16 flex flex-col md:flex-row items-center gap-10 md:gap-20 border border-apple-blue/10 bg-white/50 backdrop-blur-sm">
            ...
            <div className="flex-1 space-y-6 md:space-y-8 text-center md:text-left">
              <div className="space-y-3">
                <div className="inline-flex items-center gap-2 text-apple-blue font-black text-[10px] md:text-sm tracking-widest uppercase mb-1">
                  <Sparkles className="w-4 h-4 md:w-5 md:h-5" /> Your dedicated support
                </div>
                <h2 className="text-[34px] md:text-6xl font-black tracking-tighter text-apple-black uppercase leading-none">COACH ĐỒNG HÀNH.</h2>
                <h3 className="text-xl md:text-3xl font-black text-apple-blue uppercase tracking-tight">{COACH.name}</h3>
              </div>
              
              <div className="relative">
                <div className="absolute -left-4 top-0 text-6xl text-apple-blue/10 font-serif hidden md:block">“</div>
                <p className="text-apple-dark-gray font-bold text-lg md:text-2xl leading-relaxed tracking-tight italic">
                  {COACH.bio}
                </p>
              </div>
              
              <div className="pt-4 flex flex-wrap justify-center md:justify-start gap-4">
                <div className="bg-white px-5 py-3 rounded-2xl shadow-sm border border-black/5 flex items-center gap-3">
                  <CheckCircle2 className="text-green-500 w-5 h-5" />
                  <span className="font-bold text-sm md:text-base text-apple-black">Hỗ trợ 1:1 trực tiếp</span>
                </div>
                <div className="bg-white px-5 py-3 rounded-2xl shadow-sm border border-black/5 flex items-center gap-3">
                  <CheckCircle2 className="text-green-500 w-5 h-5" />
                  <span className="font-bold text-sm md:text-base text-apple-black">Giải đáp 24/7</span>
                </div>
              </div>
            </div>
          </div>
